...
export default function PublicBookingPage() {
  ...
  <Box>
    ...
    <Grid container spacing={isMobile ? 1 : 2}>
      {availableScreens.map((screen, idx) => (
        <Grid item xs={12} sm={6} lg={4} key={screen.id} sx={{ display: 'flex' }}>
          <Card
            sx={{
              display: 'flex',
              ...
              width: '300px',
              height: isMobile ? 260 : 320, // ✅ Fixed height for uniformity
              cursor: 'pointer',
              border: bookingForm.screen === screen.id ? 2 : 1,
              borderColor: bookingForm.screen === screen.id ? 'primary.main' : 'grey.300',
              bgcolor: bookingForm.screen === screen.id ? 'primary.50' : 'inherit',
          ...
          >
            <CardContent sx={{ flexGrow: 1, p: isMobile ? 1.5 : 2 }}>
              ...
              {screen.amenities && screen.amenities.length > 0 && (
  <Stack direction="row" flexWrap="wrap" gap={0.5} mt={1}>
    {screen.amenities.slice(0, 2).map((amenity, i) => (
      <Chip key={i} label={amenity} size="small" />
    ))}
    {screen.amenities.length > 2 && (
      <Typography variant="caption" color="text.secondary">
        +{screen.amenities.length - 2} more
      </Typography>
    )}
  </Stack>
)}
  ...
  sx={{
    ...
    overflow: 'hidden',
    height: '100%', // makes it stretch inside Grid
    display: 'flex',
    flexDirection: 'column',
  }}
>
  <CardContent sx={{ p: 0, flex: 1, display: 'flex', flexDirection: 'column' }}>
    <Grid container sx={{ flex: 1 }}>
      <Grid item xs={12} md={isMobile ? 12 : 8}>
        <Box sx={{ p: isMobile ? 2 : 3, height: '100%' }}>
